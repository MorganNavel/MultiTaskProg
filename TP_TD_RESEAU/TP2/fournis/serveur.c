...
int main(int argc, char *argv[]) {
  ...
  /* Etape 2 : Nommer la socket du seveur */
   ...
   int taille_recv;
   int res_taille_recv = recv(dsC,&taille_recv,sizeof(taille_recv),0);
   ...
   int res_recv;
   // int i=0;
   // while(nb_octet_lu<taille_recv){
   //    // printf("test");      
   //    res_recv = recv(dsC,&buffer_recv,sizeof(buffer_recv),0);
   //    if(res_recv==-1){
   //       perror("Error recv");
   //       close(dsC);
   //       close(ds);
   ...
   //       exit(1);
   //    }
   //    nb_octet_lu = nb_octet_lu + res_recv;
   //    i++;
   //    // printf("Nombre d'octet lu: %d\nEtape: %i\n",nb_octet_lu,i);
   //    printf("%s",buffer_recv);
   // }
   printf("Client: ");
   int res_recv1 = recvTCP(dsC,buffer_recv,taille_recv);
   if(res_recv1==-1){
      perror("Error send");
      close(ds);
      exit(1);
